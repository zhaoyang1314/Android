...
public class PorcBiz extends BaseBiz {
	
	//单号
	public WebResponse porcNbr(String domain,String site,String nbr,String cnNbr) {
		WebResponse wrp = getWcuCP().addParam("domain", domain).addParam("site", site).addParam("nbr", nbr).addParam("cnNbr", cnNbr)
				.execRtn("checkPorcNbr");
		return wrp;
	}
	...
	public WebResponse porcSave(String domain, String site, String loc, String part, String vend, String qty,String um, String nbr, String appName,
			 String userId, String parentid,String nums, String ntype,String lot,String isQad,String cnNbr,String mult,String fnceffdate) {
		WebResponse wrp = getWcuCP().addParam("domain", domain).addParam("site", site).addParam("loc", loc).addParam("part", part)
				...
				.addParam("lot", lot).addParam("isQad", isQad).addParam("cnNbr", cnNbr).addParam("mult", mult).addParam("fnceffdate", fnceffdate)
				.execRtn("getPorcSave");
		return wrp;
	}	
	//解锁
	...
	//关闭条码      检查条码是否在系统中存在
	public WebResponse checkScanCBar(String domain,String site,String code) {
		WebResponse wrp = getWcuCP().addParam("domain", domain).addParam("site", site).addParam("code", code)
				.execRtn("checkScanCBar");
		return wrp;
	}
	
	//关闭条码      提交关闭条码
	public WebResponse checkScanCSub(String domain,String site,String code) {
		WebResponse wrp = getWcuCP().addParam("domain", domain).addParam("site", site).addParam("code", code)
				.execRtn("checkScanCSub");
		return wrp;
	}		
