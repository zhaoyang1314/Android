...
public class DrBiz extends BaseBiz {
	
	//单号
	public WebResponse drCheckBar(String domain,String site,String code,String userId) {
		WebResponse wrp = getWcuCP().addParam("domain", domain).addParam("site", site).addParam("code", code).addParam("userId", userId)
				.execRtn("checkdrBar");
		return wrp;
	}

	//提交数据
	public WebResponse drSub(String domain, String site,String userId,String no) {
		WebResponse wrp = getWcuCP().addParam("domain", domain).addParam("site", site).addParam("userId", userId).addParam("no",no)
				.execRtn("getdrSub");
		return wrp;
	}	
