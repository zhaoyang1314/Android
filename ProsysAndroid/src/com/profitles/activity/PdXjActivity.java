...
public class PdXjActivity extends AppFunActivity{
	...
	private float qty = 0f, box_qty = 0f;
	@Override
	protected int getMainBodyLayout() {
		return R.layout.act_pdxj;
	}
	@Override
	...
	private void getScan(){
		loadDataBase(new IRunDataBaseListens() {
			...
			@Override
			public Object onGetData() {
				return biz.PdNewScanXj(domain, site, actScan.getValStr(),loc,userid,bin,actPdNbr.getValStr());
			}
			...
			public void onCallBrack(Object data) {
				...
				if(wr.isSuccess()){
					...
					box_qty = StringUtil.parseFloat(map.get("RFLOT_MULT_QTY"));
					boxQty += 1;
	...
	//提交按钮
		public boolean OnBtnSubValidata(ButtonType btnType, View v) {
			if(StringUtil.isEmpty(actPdNbr.getValStr())){
				showErrorMsg("盘点单号不能为空");
				return false;
			}
			if(StringUtil.isEmpty(actLocbin.getValStr())){
				showErrorMsg("盘点仓储不能为空");
				return false;
			}
			if(StringUtil.parseFloat(actQty.getValStr()) > box_qty ){
				showErrorMsg("输入的数量不能大于标签单箱量");
				return false;
			}
			return true;
		}
		public Object OnBtnSubClick(ButtonType btnType, View v) {
			return biz.PdSubmitXj(domain, site, actScan.getValStr(),loc,userid,bin,actPdNbr.getValStr(), actQty.getValStr());
		}	
