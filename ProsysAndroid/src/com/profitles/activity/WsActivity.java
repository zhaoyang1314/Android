...
import com.profitles.biz.WsBiz;
...
public class WsActivity extends AppFunActivity {
	...
	private MySpinner spinnerWarehouse;
	...
	protected void pageLoad() {
		...
		spinnerWarehouse = (MySpinner) findViewById(R.id.etx_Warehouse);
		...
		spinnerWarehouse.setOnItemSelectedListener(new OnItemSelectedListener() {
			@Override
	...
	public void BlurNbr(){
		loadDataBase(new IRunDataBaseListens() {
			...
			public Object onGetData() {
				return wsBiz.ws_nbr(domain, site , actNbr.getValStr());
			}
	...
	public void getWarehouse(){
		loadDataBase(new IRunDataBaseListens() {
			...
			@Override
			public Object onGetData() {
				return wsBiz.getWarehouse(domain, site , userid);
			}
			...
			public void onCallBrack(Object data) {
				...
				if (wrs.isSuccess()) {
					...
					for (int i = 0; i < dataToList.size(); i++) {
						selList.add(dataToList.get(i).get("XSLOC_LOC")+"");
					}
					String[] ub = selList.toArray(new String[selList.size()]); 
					spinnerWarehouse.addItems(ub);
				}
			}
		});
	}
	...
	public void BlurBar(){
		loadDataBase(new IRunDataBaseListens() {
			...
			@Override
			public Object onGetData() {
				return wsBiz.ws_scan(domain, site , actBar.getValStr(),actNbr.getValStr(),rfc_is_nbr_fps);
			}
					...
					if (wrs.isSuccess()) {
						...
						if(isflgn){
							...
							if(StringUtil.parseFloat(map.get("RFLOT_SCATTER_QTY"))>0){
								actBox.setText("0");
								actQty.setText(map.get("RFLOT_SCATTER_QTY")+"");
							}else{
								actBox.setText("1");
	...
	public Object OnBtnSubClick(ButtonType btnType, View v) {
		return wsBiz.ws_submit(domain, site, userid,locbin,tiaoma,vend,lbs,actQty.getValStr(),actNbr.getValStr(),cons_nbr,fnceffdate,rfc_is_nbr_fps,actPart.getValStr(),actLot.getValStr(),actUm.getValStr(), 
				spinnerWarehouse.getValStr());
	}
